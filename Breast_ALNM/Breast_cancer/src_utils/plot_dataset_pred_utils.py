...
def plot_class_preds(dataformat: str,
                     ...
                     device="cpu"):
    # 自动进行调用获取类别字典
    index_to_label, label_to_index = getClassIndices(brestDir_abs_index=2)
    # 判断是否存在测试集的图片
    ...
    net.eval()
    # inference
    #pred_df = pd.DataFrame(columns=['ID1', "ID2", "pred", "prob", 'ture_label'], index=None)
    ...
    with torch.no_grad():
        ...
        #probs, _ = torch.max(outputs_sf, dim=1)
    ...
    # print(outputs_sf.cpu().numpy().shape)
    ...
    # 多模态打印的图
        ...
        for i in range(num_imgs):
            ...
            #
            #
            #     ################################# 把测试集错误的数据集源数据放到 wrong 文件夹中 ####################################
            #     bmode_ori_path = all_testImg_path_bmode[i]
            #     bmode_dst_path = worng_dir + "ori_Data/Bmode/" +all_testImg_path_bmode[i].split("\\")[1].split("(")[1].split(")")[0] + ".jpg"
            #     if not os.path.exists(worng_dir + "ori_Data/Bmode" ):
            #         os.makedirs(worng_dir + "ori_Data/Bmode" )
            #
            #
            #     swe_ori_path = all_testImg_path_swe[i]
            #     swe_dst_path = worng_dir + "ori_Data/Swe/" + all_testImg_path_swe[i].split("\\")[1].split("(")[1].split(")")[0] + ".jpg"
            #     if not os.path.exists(worng_dir + "ori_Data/Swe" ):
            #         os.makedirs(worng_dir + "ori_Data/Swe" )
            #
            #     shutil.copy(bmode_ori_path,bmode_dst_path)
            #     shutil.copy(swe_ori_path,swe_dst_path)
            #     ##############################################################################################################
            #
            #     ID1 = all_testImg_path_swe[i].split("\\")[1].split("(")[1].split(")")[0].split("-")[0]
            #     ID2 = all_testImg_path_swe[i].split("\\")[1].split("(")[1].split(")")[0].split("-")[1]
            #     wrong_df = pd.concat(
            #         [wrong_df, pd.DataFrame([{'ID1': ID1, "ID2": ID2, 'label': label[i]}])],
            #         ignore_index=True)
            #     wrong_df.to_csv(worng_dir + "ori_Data/wrong_labels.csv", index=False)
            #
            #     fig1.savefig(worng_dir + all_testImg_path_bmode[i].split("\\")[1])
            # else:
            #
            #     ################################# 把测试集错误的数据集源数据放到 wrong 文件夹中 ####################################
            #     bmode_ori_path = all_testImg_path_bmode[i]
            #     bmode_dst_path = correct_dir + "ori_Data/Bmode/" + \
            #                      all_testImg_path_bmode[i].split("\\")[1].split("(")[1].split(")")[0] + ".jpg"
            #     if not os.path.exists(correct_dir + "ori_Data/Bmode"):
            #         os.makedirs(correct_dir + "ori_Data/Bmode")
            #
            #     swe_ori_path = all_testImg_path_swe[i]
            #     swe_dst_path = correct_dir + "ori_Data/Swe/" + \
            #                    all_testImg_path_swe[i].split("\\")[1].split("(")[1].split(")")[0] + ".jpg"
            #     if not os.path.exists(correct_dir + "ori_Data/Swe"):
            #         os.makedirs(correct_dir + "ori_Data/Swe")
            #
            #     shutil.copy(bmode_ori_path, bmode_dst_path)
            #     shutil.copy(swe_ori_path, swe_dst_path)
            #     ##############################################################################################################
            ...
            #         [correct_df, pd.DataFrame([{'ID1': ID1, "ID2": ID2, 'label': label[i]}])],
            #         ignore_index=True)
            #
            #     correct_df.to_csv(correct_dir + "ori_Data/correct_labels.csv",index=False)
            #     fig1.savefig(correct_dir + all_testImg_path_bmode[i].split("\\")[1])
